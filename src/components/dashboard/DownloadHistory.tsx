...
export const DownloadHistory = () => {
  ...
  const renderDownloadItem = (download: DownloadRecord) => {
    // Extract domain from URL for platform
    ...
    // Get video title from URL or format
    const getVideoTitle = (url: string) => {
      // Extract the last part of the URL path as a simple title
      ...
      <div key={download.id} className="py-4">
        <div className="flex items-start gap-4">
          <div className="relative h-16 w-28 flex-shrink-0 overflow-hidden rounded-md">
            <img 
              src={`https://picsum.photos/seed/${download.id}/300/200`} // Placeholder image
              alt={getVideoTitle(download.videoUrl)}
          ...
          <div className="flex-1 min-w-0">
            <p className="font-medium truncate">{getVideoTitle(download.videoUrl)}</p>
            
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1 text-xs text-muted-foreground">
              <div className="flex items-center">
                {getPlatformIcon(download.videoUrl)}
                <span className="ml-1">{getPlatformName(download.videoUrl)}</span>
              </div>
              ...
              <div className="flex items-center">
                ...
                <span className={download.status === 'completed' ? 'text-green-500' : 'text-red-500'}>
                  {download.status ? (download.status.charAt(0).toUpperCase() + download.status.slice(1)) : 'Unknown'}
                </span>
              </div>
            </div>
